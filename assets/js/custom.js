  ...
  $(".content-wrapper").on("click",".modalButtonUser", function(e) {
    ...
    $.ajax({
      ...
      data : {
        ...
        viewForm: $(this).attr('data-form')
      }
    }).done(function(data) { 
      $('#nameModal_user').find('.box-title').html(title);
      $('#nameModal_user').find('.modal-body').html(data);
      $('#nameModal_user').modal('show'); 
    })
  });

  $(".content-wrapper").on("click",".modalButtonUserAvail", function(e) {
    $.ajax({
      url : $('body').attr('data-base-url') + 'user/get_modal_available',
      method: 'post', 
      data : {
        id: $(this).attr('data-src'),      
      }
    }).done(function(data) {
      $('#nameModal_available').find('.modal-body').html(data);
      $('#nameModal_available').modal('show'); 
    })
  });

  $(".content-wrapper").on("click",".modalButtonUserFare", function(e) {  
    var title = $(this).attr('data-title');    
    $.ajax({
      url : $('body').attr('data-base-url') + 'user/get_modal_fare',
      method: 'post', 
      data : {
        id: $(this).attr('data-src'),    
      }
