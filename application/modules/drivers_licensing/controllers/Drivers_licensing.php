...
class Drivers_licensing extends CI_Controller {
  	function __construct() {
	    parent::__construct();
	    $this->load->model("Drivers_licensing_model"); 
		if(true==1){
			is_login(); 
      ...
      */
  	public function index() {   
  		if(CheckPermission("drivers_licensing", "all_read,own_read")){
			$data["view_data"]= $this->Drivers_licensing_model->get_data();
	...
	public function add_edit() {	
		...
		if($this->input->post('id')) {
			...
			$this->Drivers_licensing_model->updateRow('drivers_licensing', 'drivers_licensing_id', $this->input->post('id'), $data);
			$this->session->set_flashdata('message', 'Your data updated Successfully..');
      		redirect('drivers_licensing');
		} else { 
			unset($data['submit']);
			unset($data['save']);
			...
			$this->Drivers_licensing_model->insertRow('drivers_licensing', $data);
	...
	public function get_modal() {
		if($this->input->post('id')){
			$data['data']= $this->Drivers_licensing_model->Get_data_id($this->input->post('id'));
	...
	/**
      ...
      */
  	public function delete($ids) {
		...
		foreach ($idsArr as $key => $value) {
			$this->Drivers_licensing_model->delete_data($value);		
		}
		redirect(base_url().'drivers_licensing', 'refresh');
  	}
  	/**
      ...
      */
  	public function delete_data($id) { 
		$this->Drivers_licensing_model->delete_data($id);
	...
	/**
      ...
      */
  	public function ajx_data(){
		$primaryKey = 'drivers_licensing_id';
		$table 		= 'drivers_licensing';
		$columns 	= array(
array( 'db' => 'drivers_licensing_id', 'dt' => 0 ),
array( 'db' => 'drivers_licensing_license', 'dt' => 1 ),
array( 'db' => 'drivers_licensing_id', 'dt' => 2 )
);
		$joinQuery 	= '';
      ...
      */
  	public function getFilterdata(){
		...
		if($this->input->post('dateRange')) {
			...
			$where = " DATE_FORMAT(`drivers_licensing`.`".$this->input->post('colName')."`, '%Y/%m/%d') >= '".date('Y/m/d', strtotime($date[0]))."' AND  DATE_FORMAT(`drivers_licensing`.`".$this->input->post('colName')."`, '%Y/%m/%d') <= '".date('Y/m/d', strtotime($date[1]))."' ";
		}
		$data["view_data"]= $this->Drivers_licensing_model->get_data($where);
