...
class User extends CI_Controller {
  ...
  public function get_modal_fare(){
        is_login();
        if($this->input->post('id')){
            $data['fareDatas'] = getAllDataTable('fare',$this->input->post('id'),'users_id'); 
...
public function add_edit_available_api(){
 ...
 public function searchDriver(){
   header("Access-Control-Allow-Origin: *");
    ...
    if($this->input->post('driver_number')) {          
            $data=getDataByid('users',$this->input->post('driver_number'),'users_id');            
            if(!empty($data)){
             $make=getDataByid('make',$data->make,'make_id');
             $model=getDataByid('model',$data->model,'model_id');   
             $data->make_value=!empty($make->make_value)?$make->make_value:'';
             $data->model_value=!empty($model->model_value)?$model->model_value:'';  
            }
 ...
 public function CallAPi($url, $info){
   ...
   public function favDriversInfo(){
    ...
    // echo"<PRE>";print_r($data);print_r($outputData);die;        
     echo json_encode($outputData);
   }
   ...
   public function add_edit_fare(){
    ...
    foreach($datas['d_fare_name'] as $data){
     ...
     if(!empty($datas['m_fare_name'])){
       $savedata['fare_name']     =  $datas['m_fare_name'];
       $savedata['fare_duration'] =  implode('/',$datas['m_fare_duration']);
       $savedata['fare_price']    =  implode('/',$datas['m_fare_price']);
       $savedata['fare_status']   =  'mile';
       $savedata['users_id']      =  $this->user_id;
       $savedata['is_deleted']    =  0;

         if(!empty($datas['m_fare_id'])){
            $this->User_model->updateRow('fare', 'fare_id', $datas['m_fare_id'], $savedata);
            $this->session->set_flashdata('message', 'Your data updated Successfully..');
         }else{
            $this->User_model->insertRow('fare', $savedata);
            $this->session->set_flashdata('message', 'Your data inserted Successfully..');
    ...
    $res = $this->User_model->get_data_by('model', $this->input->post('make_id'), 'make_id');
    $option ='';
