...
public function add_edit_available_api(){
 ...
 public function deviceDriver(){
     ...
     if($this->input->post('driver_id') && $this->input->post('client_id')) { 
         ...
         foreach($subs as $sub){
               if($sub->subscription_price > 0 ){
                 ...
                 $free_subs = !empty($subscription_driver[0]->subscription_driver)?$subscription_driver[0]->subscription_driver:3;
               }
              } 
             ...
             foreach($subs as $sub){
               if($sub->subscription_price > 0 ){  
                  ...
                  $subscription_driver = getAllDataTables('passenger_subs',$data['client_id'],'client_id');     
                  $driver = !empty($subscription_driver[0]->subscription_driver)?$subscription_driver[0]->subscription_driver+$free_subs:$free_subs;
                  if($driver <= count($favourite_driver)){                   
                    $datas['p_subs'] = 1;  
  ...
  public function subcriptionPassengerCancel(){ 
    ...
    $favourite_driver=  getAllDataTables('favourite_driver',50,'client_id'); 
    $subscription_driver=!empty($subscription_price[0]->subscription_driver)?$subscription_price[0]->subscription_driver:3;

    for($i=0;$i<count($favourite_driver)-$subscription_driver;$i++){   
 ...
 public function CallAPi($url, $info){
   ...
   public function submitForm(){      
        ...
        if($this->input->post('users_id')) {
            ...
            $this->User_model->updateRow('users', 'users_id', $id, $dataId);
	...
	public function sendGetClientBooking($client_id=''){
		   ...
		   foreach ($dataOutput as $key=>$val)
		   {
			...
			$dataOutput1[$key]['booking_time_slot_date_time_slot']= date ('D d/m/y h:i:A', strtotime(isset($val['booking_time_slot_date_time_slot'])?$val['booking_time_slot_date_time_slot']:date()));
            $dataOutput1[$key]['fullname']= isset($val['fullname'])?$val['fullname']:'None';
			$dataOutput1[$key]['first_name']= isset($val['first_name'])?$val['first_name']:'None';
			$dataOutput1[$key]['last_name']= isset($val['last_name'])?$val['last_name']:'None';
			$dataOutput1[$key]['amount']= isset($val['amount'])?$val['amount']:'None';
			$dataOutput1[$key]['client_collection_point']= isset($val['client_collection_point'])?$val['client_collection_point']:'';
			$dataOutput1[$key]['drop_point']= isset($val['drop_point'])?$val['drop_point']:'';
			$dataOutput1[$key]['profile_pic']= isset($val['profile_pic'])?$baseURL.''.$val['profile_pic']:$baseURL.'user.png';
			$dataOutput1[$key]['booking_time_slot_driver_id']= isset($val['booking_time_slot_driver_id'])?$val['booking_time_slot_driver_id']:'';
			$dataOutput1[$key]['booking_time_slot_book_id']= isset($val['booking_time_slot_book_id'])?$val['booking_time_slot_book_id']:'';
			$dataOutput1[$key]['booking_time_slot_status']= isset($val['booking_time_slot_status'])?$val['booking_time_slot_status']:'';
   ...
   public function getrattingClientById($data=array()){
    ...
    echo json_encode($dataOutput); 
     ...
     public function getRattingsAndCommentsDriverId($driver_id='',$type='')
      ...
      header("Access-Control-Allow-Origin: *");
      $baseURL = base_url().'assets/images/';
      if(empty($driver_id)){$data = $this->input->post(); $driver_id = isset($data['driver_id'])?$data['driver_id']:'';}
      $dataOutput=$dataOutput1=array();$sumArray = array();$amount = array(); $i=0;
      ...
      foreach($dataOutput as $bank) {
       ...
       if ($index < 0) {$amount[] = $bank; } else {
       $amount[$index]['rate'] +=  $bank['rate']; $amount[$index]['total'] =  count($dataOutput); }$i++;
       $amount['comments'][] = $bank['ratting_Comments'];
      }
      if(!empty($amount)){$res =  isset($amount[0])?$amount[0]:array('rate'=>'','total'=>'');
      $ratting = $res['rate']/$res['total']; $ratting = round($ratting, 0);
      $dataOutput1 =array('myRating'=>$ratting,'comments'=>$amount['comments']);
      if($type=='return'){return $dataOutput1;}else{echo json_encode($dataOutput1);}
      }else{ if($type=='return'){return array('myRating'=>0,'comments'=>array());}else{echo json_encode(array('myRating'=>0,'comments'=>array()));}}
         }
          
          function ratting_existsDriver($bankname, $array) { $result = -1;
          for($i=0; $i<sizeof($array); $i++) {if ($array[$i]['ratting_driver_id'] == $bankname) { $result = $i;break; }}
          return $result;
         }
