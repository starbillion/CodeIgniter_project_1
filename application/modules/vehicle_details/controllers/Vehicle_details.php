...
class Vehicle_details extends CI_Controller {
      ...
      */
  	public function index() {   
  		if(CheckPermission("vehicle_details", "all_read,own_read")){
			$data["view_data"]= $this->Vehicle_details_model->get_data();
	...
	public function add_edit() {	
		...
		if($this->input->post('id')) {
			...
			$this->Vehicle_details_model->updateRow('vehicle_details', 'vehicle_details_id', $this->input->post('id'), $data);
			$this->session->set_flashdata('message', 'Your data updated Successfully..');
      		redirect('vehicle_details');
		} else { 
			unset($data['submit']);
			unset($data['save']);
			...
			$this->Vehicle_details_model->insertRow('vehicle_details', $data);
	...
	public function get_modal() {
		if($this->input->post('id')){
			$data['data']= $this->Vehicle_details_model->Get_data_id($this->input->post('id'));
	...
	/**
      ...
      */
  	public function delete($ids) {
		...
		foreach ($idsArr as $key => $value) {
			$this->Vehicle_details_model->delete_data($value);		
		}
		redirect(base_url().'vehicle_details', 'refresh');
  	}
  	/**
      ...
      */
  	public function delete_data($id) { 
		$this->Vehicle_details_model->delete_data($id);
	...
	/**
      ...
      */
  	public function ajx_data(){
		$primaryKey = 'vehicle_details_id';
		$table 		= 'vehicle_details';
		$columns 	= array(
array( 'db' => 'vehicle_details_id', 'dt' => 0 ),
array( 'db' => 'vehicle_details_model', 'dt' => 1 ),
array( 'db' => 'vehicle_details_make', 'dt' => 2 ),
array( 'db' => 'vehicle_details_registration', 'dt' => 3 ),
array( 'db' => 'vehicle_details_id', 'dt' => 4 )
);
		$joinQuery 	= '';
      ...
      */
  	public function getFilterdata(){
		...
		if($this->input->post('dateRange')) {
			...
			$where = " DATE_FORMAT(`vehicle_details`.`".$this->input->post('colName')."`, '%Y/%m/%d') >= '".date('Y/m/d', strtotime($date[0]))."' AND  DATE_FORMAT(`vehicle_details`.`".$this->input->post('colName')."`, '%Y/%m/%d') <= '".date('Y/m/d', strtotime($date[1]))."' ";
		}
		$data["view_data"]= $this->Vehicle_details_model->get_data($where);
