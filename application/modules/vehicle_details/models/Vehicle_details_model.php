...
class Vehicle_details_model extends CI_Model {       
	...
	/**
      ...
      * @param : $id is value of vehicle_details_id
      */
	public function Get_data_id($id='') {
		 $this->db->select('*');
		 $this->db->from('vehicle_details');
		 $this->db->where('vehicle_details_id' , $id);
		 $query = $this->db->get();
	...
	public function get_data($con=NULL) {
		if(CheckPermission('vehicle_details', "own_read") && CheckPermission('vehicle_details', "all_read")!=true){
			if($con != '') {
				$con .= " AND "; 
			}
			$con .= "  (`vehicle_details`.`user_id` = '".$this->user_id."') ";
		}
